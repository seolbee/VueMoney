...
export default {
    ...
    methods : {
        showBalnce(name) {
            let filter = this.data.filter(x=> x.print_content == x.name);
            this.$router.push({path : '/balance', params:{filter}});
        },
        async getStatistics() {
            let res = await axios.get(`/bank/history/${this.finTechNum}`);
            this.data = res.data.data;
            this.use_list.forEach(x=>{
                let filter = this.data.filter(f=> f.print_content == x.name).map(m => m.tran_amt *= 1);
                x.price = filter.reduce((initValue, currentValue)=> initValue + currentValue);
            });
